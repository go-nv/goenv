...
Use `search`/`read` to verify claims against the actual codebase before
accepting them — don't take the plan's word for what the code does. Use `web`
only to check current facts you can't verify locally (e.g., whether a claimed
CVE/advisory is real, whether a library's maintenance status is as described,
current S-SDLC/OWASP guidance) — never to look up how to write the fix.

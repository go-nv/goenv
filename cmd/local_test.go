...
func TestLocalCommand(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			args: []string{"1.2.3"},
			setup: func(t *testing.T, root string) (string, func()) {
				createTestVersion(t, root, "1.2.3")
				workDir := filepath.Join(root, "project")
				if err := os.MkdirAll(workDir, 0755); err != nil {
					t.Fatalf("failed to create workdir: %v", err)
				}
		...
		},
		{
			...
			args: []string{"latest"},
			setup: func(t *testing.T, root string) (string, func()) {
				createTestVersion(t, root, "1.10.10")
				createTestVersion(t, root, "1.10.9")
				createTestVersion(t, root, "1.9.10")
				workDir := filepath.Join(root, "project")
				if err := os.MkdirAll(workDir, 0755); err != nil {
					t.Fatalf("failed to create workdir: %v", err)
				}
		...
		},
		{
			...
			args: []string{"1"},
			setup: func(t *testing.T, root string) (string, func()) {
				createTestVersion(t, root, "1.2.10")
				createTestVersion(t, root, "1.2.9")
				createTestVersion(t, root, "4.5.6")
				workDir := filepath.Join(root, "project")
				if err := os.MkdirAll(workDir, 0755); err != nil {
					t.Fatalf("failed to create workdir: %v", err)
				}
		...
		},
		{
			...
			setup: func(t *testing.T, root string) (string, func()) {
				createTestVersion(t, root, "1.2.10")
				createTestVersion(t, root, "1.2.9")
				createTestVersion(t, root, "1.3.11")
				createTestVersion(t, root, "4.5.2")
				workDir := filepath.Join(root, "project")
				if err := os.MkdirAll(workDir, 0755); err != nil {
					t.Fatalf("failed to create workdir: %v", err)
				}
		...
		},
		{
			...
			args: []string{"1.2"},
			setup: func(t *testing.T, root string) (string, func()) {
				createTestVersion(t, root, "1.1.2")
				createTestVersion(t, root, "1.2.9")
				createTestVersion(t, root, "1.2.10")
				createTestVersion(t, root, "1.3.11")
				createTestVersion(t, root, "2.1.2")
				workDir := filepath.Join(root, "project")
				if err := os.MkdirAll(workDir, 0755); err != nil {
					t.Fatalf("failed to create workdir: %v", err)
				}
		...
		},
		{
			...
			setup: func(t *testing.T, root string) (string, func()) {
				createTestVersion(t, root, "1.2.9")
				createTestVersion(t, root, "4.5.10")
				workDir := filepath.Join(root, "project")
				if err := os.MkdirAll(workDir, 0755); err != nil {
					t.Fatalf("failed to create workdir: %v", err)
				}

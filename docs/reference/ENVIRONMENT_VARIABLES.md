...
`GOENV_FILE_ARG` | | Automatically set by shims when executing `go*` commands with file arguments (paths containing `/`). Used by hooks to detect file-based operations.

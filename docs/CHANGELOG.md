...
- **🩹 A literal `~` in `GOPATH` broke every shimmed `go` command** - goenv prepends a per-version path to `GOPATH` and re-exports it on every shimmed call, so a profile that left a literal `~` (e.g. a quoted `export GOPATH="~/go"`, which the shell does not expand) was passed through to `go`, which rejects it: "GOPATH entry cannot start with shell metacharacter '~'". goenv now expands `~`/`$HOME` in each inherited `GOPATH` entry — in both the shell integration and the exec path — before handing it back to `go`. Expansion only rewrites entries; it never drops or blanks a value goenv can't fully resolve, so an unusable path still reaches `go` and produces `go`'s own error rather than being silently replaced. The same tilde/`$VAR` expansion is applied wherever goenv hands a path to `go` or reads a path from the environment: `GOCACHE`, `GOMODCACHE`, `GOBIN`, `GOENV_GOCACHE_DIR` (shim exec and `goenv tools install`), the `GOENV_HOOKS_CONFIG`/`GOENV_HOOKS_LOG`/`GOENV_ROOT` config paths and hook log/command/disk paths, and all SBOM path flags and positional file arguments. Windows `~\`, `%VAR%`, and `%USERPROFILE%` forms are handled by the same normalizer and covered by unit tests (the Unix behavior is additionally covered end-to-end).

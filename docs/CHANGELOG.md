...
- **🩹 A literal `~` in `GOPATH` broke every shimmed `go` command** - goenv prepends a per-version path to `GOPATH` and re-exports it on every shimmed call, so a profile that left a literal `~` (e.g. a quoted `export GOPATH="~/go"`, which the shell does not expand) was passed through to `go`, which rejects it: "GOPATH entry cannot start with shell metacharacter '~'". goenv now normalizes inherited `GOPATH` entries — expanding `~`/`$HOME` and dropping non-absolute entries Go can't use — in both the shell integration and the exec path, on all platforms. The same expansion was applied everywhere goenv hands a path to `go` or reads a path from the environment: `GOCACHE`, `GOMODCACHE`, `GOBIN`, `GOENV_GOCACHE_DIR` (shim exec and `goenv tools install`), and the `GOENV_HOOKS_CONFIG`/`GOENV_HOOKS_LOG`/`GOENV_ROOT` config paths. SBOM path flags/args (`--output`, `--dir`, `--binary`, `--policy`, `--key`) now expand `~` too.
